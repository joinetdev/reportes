...
if(isset($_GET['order_id']) &&  $_GET['order_id'] != "" &&  $_GET['order_id'] >= 0 ){
        ...
        foreach($items as $item){
            ...
            foreach($articulos as $articulo){
                if( in_array($sku, $articulo, true) ){
					...
					for ($x = 0; $x <= 100; $x++) {										
						if( $articulo['meta_data'][$x]['key'] == 'MBPOS_STOCK_COLON' ){
							$stock_colon =   $articulo['meta_data'][$x]['value'];						
						}
						if( $articulo['meta_data'][$x]['key'] == 'MBPOS_STOCK_COTILLA' ){
							$stock_cotilla =  $articulo['meta_data'][$x]['value'];
						}
						if( $articulo['meta_data'][$x]['key'] == 'MBPOS_STOCK_CORONA' ){
							$stock_corona =   $articulo['meta_data'][$x]['value'];
						}
					}
    ...
    ?>
        <tr>
            ...
            <td><?=  number_format($pro['cantidad'],2); ?></td>
            <td>$<?= number_format($pro['costo'],2); ?></td>
            <td>$<?= number_format($pro['total'],2); ?></td>
			
			<td><b><?= number_format($pro['colon'],0); ?></b></td>
			<td><b><?= number_format($pro['cotilla'],0); ?></b></td>
			<td><b><?= number_format($pro['corona'],0); ?></b></td>
        </tr>
    <?php
    }
    ?>
        ...
        <tr>
            ...
            <td colspan="4">$<?= number_format($envio = $order['shipping_total'],2); ?></td>
        </tr>
        <tr>
            <td colspan="2"></td>
            <td></td>
            ...
            <td colspan="4"><b>$<?= number_format($total = $subtotal + $envio,2); ?></b></td>
        </tr>
